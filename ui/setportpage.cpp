...
void setportpage::init()
{
    ...
    networkTestLabel->move(200,350); //move(200,200);
    ...
    QLabel *goodsTestLabel = new QLabel(testPage);
    goodsTestLabel->resize(147,35);
    goodsTestLabel->move(170,200);  //move(170,350);
    goodsTestLabel->setStyleSheet("background-image: url(:/page/images/manage/word15.png)");
   ...
   // setIPAddrLable->setText("网络通讯测试地址");
    ...
    network_lineEdit->move(330,350); //move(330,200);
    ...
    networkBtn->move(580,340); //move(580,185);
    ...
    goodsCheck_lineEdit = new CKeyBoardLineEdit(testPage);
    goodsCheck_lineEdit->resize(200,40);
    goodsCheck_lineEdit->move(330,200); //move(330,350);
    goodsCheck_lineEdit->setMaxLength(4);

    goodsBtn = new QPushButton(testPage);
    goodsBtn->resize(99,62);
    goodsBtn->move(580,185); //move(580,340);
    goodsBtn->setFlat(true);
    goodsBtn->setFocusPolicy(Qt::NoFocus);
    goodsBtn->setStyleSheet("background-color:transparent;background-image:url(:/page/images/take/okBtn.png)");
...
//    connect(cancelbtn,SIGNAL(clicked()),this,SLOT(close()));
    ...
    connect(goodsBtn,SIGNAL(clicked()),this,SLOT(checkGoods_slot()));
    ...
    connect(goodsCheck_lineEdit,SIGNAL(btnPress()),this,SIGNAL(resetTime()));
...
void setportpage::checkGoods_slot()
{
    emit resetTime();
    goodsBtn->setEnabled(false);
    QString retBox;
    QString waitCheckBox = goodsCheck_lineEdit->text().trimmed();
    QRegExp rxpasswd("^0\\d{3}");
    if(rxpasswd.exactMatch(waitCheckBox))
    {
        retBox.clear();
        qint8 cabinetId = waitCheckBox.left(2).toInt();
        qint8 cellId = waitCheckBox.right(2).toInt();
        for(int i = cellId; i > 0; i--)
        {
            if(lockoperation.queryGoods(cabinetId,i) == 0)
            {
                QString temp = waitCheckBox.left(2) + QString("%1").arg(i,2,10,QChar('0')) + ",";
                retBox = retBox + temp;
            }
        }
        if(retBox.isEmpty())
        {
            myMessageBox.myPrompt(QMessageBox::Warning,tr("提示"),tr("物品检测条正常"));
        }
        else
        {
            retBox.chop(1);
            myMessageBox.myPrompt(QMessageBox::Warning,tr("提示"),QString("%1箱门损坏").arg(retBox));
        }
    }
    else
    {
