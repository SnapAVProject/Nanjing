...
namespace Kompex
{
    ...
    SQLiteDatabase::SQLiteDatabase() :
        mDatabaseHandle(0), mIsMemoryDatabaseActive(false)
    {
    }
    ...
    void
    SQLiteDatabase::Open(const char *filename, int flags, const char *zVfs)
    {
        ...
        if (sqlite3_open_v2(filename, &mDatabaseHandle, flags, zVfs) != SQLITE_OK)
            KOMPEX_EXCEPT(
            sqlite3_errmsg(mDatabaseHandle));

        mDatabaseFilename = std::string(filename);
    }

    void
    SQLiteDatabase::Open(const std::string &filename, int flags, const char *zVfs)
    {
        // close old db, if one exist
        if (mDatabaseHandle)
            Close();
    ...
    void
    SQLiteDatabase::Open(const wchar_t *filename)
    {
        // close old db, if one exist
        if (mDatabaseHandle)
            Close();
    ...
    void
    SQLiteDatabase::TraceOutput(void *ptr, const char *sql)
    {
        std::cout << "trace: " << sql << std::endl;
    }

    void
    SQLiteDatabase::ProfileOutput(void* ptr, const char* sql, sqlite3_uint64 time)
    {
    ...
    SQLiteDatabase::MoveDatabaseToMemory()
    {
        if (!mIsMemoryDatabaseActive)
        {
            ...
            sqlite3_exec(mDatabaseHandle, "SELECT sql FROM sqlite_master WHERE sql NOT NULL",
                    &Kompex::SQLiteDatabase::ProcessDDLRow, memoryDatabase, 0);
            ...
            sqlite3_exec(memoryDatabase, "SELECT name FROM origin.sqlite_master WHERE type='table'",
                    &Kompex::SQLiteDatabase::ProcessDMLRow, memoryDatabase, 0);
    ...
    int
    SQLiteDatabase::ProcessDDLRow(void *db, int columnsCount, char **values, char **columns)
    {
        if (columnsCount != 1)
        {
    ...
    int
    SQLiteDatabase::ProcessDMLRow(void *db, int columnsCount, char **values, char **columns)
    {
        if (columnsCount != 1)
        {
    ...
    void
    SQLiteDatabase::SaveDatabaseFromMemoryToFile(const std::string &filename)
    {
        if (mIsMemoryDatabaseActive)
        {
            ...
            if (sqlite3_open_v2(file.c_str(), &fileDatabase, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, 0)
                    != SQLITE_OK)
